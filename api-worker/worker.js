...
function assertOrigin(request, env) {
  ...
  if ((request.headers.get('Origin') || '') !== allowed) throw new Error('Origin is not allowed.');
}
async function bridgeFetchOnce(env, action, args, timeoutMs = 8000) {
  ...
  let r;
  try {
    r = await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bridgeSecret:secret,action,args}),redirect:'follow',signal:controller.signal});
  } catch (err) {
  ...
  try { data = JSON.parse(text); }
    ...
    if (!r.ok) throw new Error('Apps Script bridge returned HTTP ' + r.status + '.');
    if (type.includes('text/html')) throw new Error('Apps Script bridge returned HTML instead of JSON.');
...
async function callAppsScript(env, action, args, options = {}) {
  ...
  for (let attempt=0; attempt<=retries; attempt++) {
    try { return await bridgeFetchOnce(env, action, args, timeoutMs); }
    catch (err) { lastErr = err; if (attempt>=retries) break; await sleep(400*(attempt+1)); }
  }
...
async function getGoogleAccessToken(env, forceFresh = false) {
  ...
  const r = await callAppsScript(env,'getDriveUploadTokenV5',[],{retries:1,timeoutMs:8000});
...
async function driveFetch(env,url,init={},retry401=true){
  ...
  if(r.status===401&&retry401){token=await getGoogleAccessToken(env,true);r=await fetch(url,{...init,headers:{...(init.headers||{}),Authorization:`Bearer ${token}`}})}
  return r;
}
async function getDriveFile(env,fileId){
  ...
  const data=await r.json();if(!r.ok||!data.id)throw new Error('Could not verify the reserved Google Drive photo file.');return data;
}
async function uploadDriveMedia(env,fileId,file,mime){
  const r=await driveFetch(env,`https://www.googleapis.com/upload/drive/v3/files/${encodeURIComponent(fileId)}?uploadType=media&supportsAllDrives=true&fields=id,name,mimeType,size,webViewLink,parents`,{method:'PATCH',headers:{'Content-Type':mime||file.type||'application/octet-stream'},body:file.stream()});
  const data=await r.json();if(!r.ok||!data.id)throw new Error('Google Drive did not finish saving the photo.');return data;
}
async function ensureDrivePhoto(env,prep,file){
  const fileId=String(prep&&prep.fileId||'');if(!fileId)throw new Error('Apps Script did not reserve a Drive file for this upload.');
  let existing=await getDriveFile(env,fileId);const currentSize=Number(existing.size||0);
  if(currentSize!==Number(file.size||0)||currentSize===0)existing=await uploadDriveMedia(env,fileId,file,prep.mime||file.type);
  if(Number(existing.size||file.size||0)<=0)throw new Error('Google Drive did not confirm the photo bytes.');
  return existing;
}
async function handleAction(request,env){
  const body=await request.json(),action=String(body.action||'');if(!ACTION_ALLOWLIST.has(action))throw new Error('API action is not allowed.');
  return await callAppsScript(env,action,Array.isArray(body.args)?body.args:[]);
}
async function confirmPhotoMetadata(env,code,p){
  return await callAppsScript(env,'commitExternalPhotoFastV5',[code,p],{retries:1,timeoutMs:6000});
}
async function handlePhotoConfirm(request,env){
  const body=await request.json();
  const code=String(body.accessCode||'').trim();
  const p=body.commitPayload||{};
  const result=await confirmPhotoMetadata(env,code,p);
  return {...result,ok:true,pendingMetadata:false};
}
async function handlePhoto(request,env,ctx){
  const form=await request.formData();const file=form.get('photoFile');
  ...
  const p={storeKey:String(form.get('storeKey')||''),photoType:String(form.get('photoType')||''),addAnother:String(form.get('addAnother')||'')==='1',uploadToken:String(form.get('uploadToken')||''),originalName:String(file.name||''),mime:String(file.type||'application/octet-stream'),size:Number(file.size||0)};
  const prep=await callAppsScript(env,'prepareExternalPhotoV5',[code,p],{retries:1,timeoutMs:9000});
  if(prep&&prep.alreadyCommitted)return {...prep,ok:true,pendingMetadata:false,recovered:true};
  const drive=await ensureDrivePhoto(env,prep,file);
  const commitPayload={...p,type:prep.type,folderId:prep.folderId,fileName:prep.fileName,fileId:drive.id};
  try {
    const committed=await confirmPhotoMetadata(env,code,commitPayload);
    return {...committed,ok:true,pendingMetadata:false};
  } catch (err) {
    if(ctx&&ctx.waitUntil){
      ctx.waitUntil((async()=>{
        for(let i=0;i<3;i++){try{await confirmPhotoMetadata(env,code,commitPayload);return;}catch(_){await sleep(1500*(i+1));}}
      })());
    }
    return {ok:true,pendingMetadata:true,driveVerified:true,fileId:drive.id,url:drive.webViewLink||'',name:drive.name||prep.fileName,folderId:prep.folderId,type:prep.type,baseType:prep.baseType,isExtra:prep.isExtra,bytes:Number(drive.size||file.size||0),message:'Photo is safely saved in Drive. Final record sync is continuing.',resume:{accessCode:code,commitPayload}};
  }
}
async function handleHealth(request,env){
  const bridge=await callAppsScript(env,'getBridgeHealthV5',[],{retries:1,timeoutMs:7000});
  return jsonResponse(request,env,{ok:true,workerVersion:'5.0.1',workerBuild:'5.0.3-drive-first',bridge});
}
...
export default {
  async fetch(request,env,ctx){
    ...
    try{
      assertOrigin(request,env);const url=new URL(request.url);
      ...
      if(url.pathname==='/api/photo')return jsonResponse(request,env,{ok:true,result:await handlePhoto(request,env,ctx)});
      if(url.pathname==='/api/photo/confirm')return jsonResponse(request,env,{ok:true,result:await handlePhotoConfirm(request,env)});
      return jsonResponse(request,env,{ok:false,error:'Not found.'},404);
    }catch(err){return jsonResponse(request,env,{ok:false,error:String(err&&err.message?err.message:err)},400);}
  }
};
