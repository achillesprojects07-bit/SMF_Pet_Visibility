...
async function handlePhoto(request,env,ctx){
  ...
  let prep;
  try{[prep]=await Promise.all([callAppsScript(env,'prepareExternalPhotoV5',[code,p],{retries:1,timeoutMs:15000}),getGoogleAccessToken(env,false)]);}catch(err){if(/timed out/i.test(String(err&&err.message||'')))throw new Error('Photo setup timed out before Drive could save the file. Please retry this photo once.');throw err;}
...
async function handleHealth(request,env){const bridge=await callAppsScript(env,'getBridgeHealthV5',[],{retries:1,timeoutMs:7000});return jsonResponse(request,env,{ok:true,workerVersion:'5.0.1',workerBuild:'5.0.5-drive-first',bridge});}
