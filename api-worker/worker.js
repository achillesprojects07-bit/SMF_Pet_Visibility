...
async function getGoogleAccessToken(env,forceFresh=false){const now=Date.now();if(!forceFresh&&cachedGoogleToken&&cachedGoogleToken.expiresAt>now+60000)return cachedGoogleToken.token;const r=await callAppsScript(env,'getDriveUploadTokenV5',[],{retries:1,timeoutMs:12000}),token=String(r&&r.accessToken||'');if(!token)throw new Error('Apps Script did not provide a Google Drive upload token.');cachedGoogleToken={token,expiresAt:now+35*60*1000};return token;}
...
async function confirmPhotoMetadata(env,code,p){return await callAppsScript(env,'commitExternalPhotoFastV5',[code,p],{retries:0,timeoutMs:7000});}
...
async function handlePhotoPreflight(request,env){
  const body=await request.json(),code=String(body.accessCode||'').trim();
  const p={storeKey:String(body.storeKey||''),photoType:String(body.photoType||''),addAnother:!!body.addAnother,uploadToken:String(body.uploadToken||''),originalName:String(body.originalName||''),mime:String(body.mime||'application/octet-stream'),size:Number(body.size||0)};
  let prep;
  try{prep=await callAppsScript(env,'prepareExternalPhotoUploadV5',[code,p],{retries:1,timeoutMs:25000});}
  catch(err){if(/timed out/i.test(String(err&&err.message||'')))throw new Error('Secure photo setup is taking too long. Please try once more before sending the photo.');throw err;}
  const token=String(prep&&prep.accessToken||'');if(!token)throw new Error('Google Drive authorization was not returned.');cachedGoogleToken={token,expiresAt:Date.now()+35*60*1000};
  const shell=await createDriveFile(env,String(prep.folderId||''),String(prep.fileName||''),String(prep.mime||p.mime||'image/jpeg'));
  const sealedToken=await sealText(env,token);
  const payload={accessCode:code,storeKey:p.storeKey,photoType:p.photoType,addAnother:!!p.addAnother,uploadToken:p.uploadToken,originalName:p.originalName,mime:String(prep.mime||p.mime||''),size:p.size,type:String(prep.type||''),baseType:String(prep.baseType||p.photoType||''),isExtra:!!prep.isExtra,folderId:String(prep.folderId||''),fileName:String(prep.fileName||''),fileId:String(shell.id||''),sealedDriveToken:sealedToken,expiresAt:Date.now()+10*60*1000};
  return {ok:true,preflight:await signPreflight(env,payload),fileReserved:true};
}
...
async function backgroundConfirm(env,code,commitPayload){for(let i=0;i<5;i++){try{return await confirmPhotoMetadata(env,code,commitPayload);}catch(_){await sleep(1200*(i+1));}}return null;}
async function handlePhoto(request,env,ctx){
  ...
  let prep;
  const rawTicket=String(form.get('preflight')||'').trim();
  if(rawTicket){
    let ticket;try{ticket=JSON.parse(rawTicket);}catch(_){throw new Error('Photo upload preflight is unreadable.');}
    const signed=await verifyPreflight(env,ticket);
    if(String(signed.accessCode||'')!==code||String(signed.storeKey||'')!==p.storeKey||String(signed.photoType||'')!==p.photoType||String(signed.uploadToken||'')!==p.uploadToken||!!signed.addAnother!==!!p.addAnother||Number(signed.size||0)!==Number(file.size||0))throw new Error('Photo upload preflight does not match this photo.');
    const token=await openText(env,signed.sealedDriveToken);if(!token)throw new Error('Photo upload credential could not be restored.');cachedGoogleToken={token,expiresAt:Date.now()+30*60*1000};
    prep={...signed,accessCode:undefined,sealedDriveToken:undefined,expiresAt:undefined};
  }else{
    try{prep=await callAppsScript(env,'prepareExternalPhotoUploadV5',[code,p],{retries:1,timeoutMs:25000});}
    catch(err){if(/timed out/i.test(String(err&&err.message||'')))throw new Error('Photo setup timed out before Drive could save the file. Please retry this photo once.');throw err;}
    const token=String(prep&&prep.accessToken||'');if(token)cachedGoogleToken={token,expiresAt:Date.now()+35*60*1000};
  }
  ...
  const resume=await signResume(env,code,commitPayload);
...
async function handleHealth(request,env){return jsonResponse(request,env,{ok:true,workerVersion:'5.0.1',workerBuild:'5.0.8-two-stage-preflight',bridge:{ok:true,bridge:'SMF_API_V5',version:'5.0.1',mode:'LIVE'}});}
