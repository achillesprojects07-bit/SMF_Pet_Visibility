...
async function syncPhotoRow(env,tx,store,userName){
  ...
  const marker='V6_UPLOAD:'+tx.upload_id; for(let i=1;i<rows.length;i++){ if(String(rows[i][idx['Notes']]||'')===marker && String(rows[i][idx['Store Key']]||'')===tx.store_key)return {duplicate:true,row:i+1}; }
  const out=new Array(headers.length).fill(''); const set=(h,v)=>out[idx[h]]=v; set('Environment',String(env.SMF_MODE||'LIVE').toUpperCase());set('Store Key',tx.store_key);set('Store Name',store.name);set('Team',store.team);set('Photo Type',tx.photo_type);set('File ID',tx.file_id);set('File Name',tx.file_name);set('File URL',tx.file_url);set('Folder ID',tx.folder_id);set('Active',true);set('Uploaded At',nowIso());set('Uploaded By',userName);set('Guide Used',store.guide||'');set('Notes',marker); await sheetAppend(env,'V4_PHOTOS!A:ZZ',out);
