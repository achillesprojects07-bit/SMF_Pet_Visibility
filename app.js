(() => {
  ...
  function renderStore(){
    ...
    $('storeBody').innerHTML=`<section class="card storeHero"><div><h1>${esc(s.name)}</h1><div class="small">${esc(s.storeId||'')} • ${esc(s.team)} • ${esc(s.area)} • ${esc(s.category)}</div><p>${esc(s.address||'')}</p></div><span class="badge ${statusClass(storeStatus)}">${esc(statusLabel(storeStatus))}</span></section>
      ...
      <section class="card"><div class="sectionHead"><div><h2>POE Photos</h2><div class="small">${uploadedMain} of ${req.length} required main photos uploaded</div></div></div><div class="photoGuide">Take/choose the photo normally. Once Drive confirms the file, the app will finish the POE record without re-uploading the image.</div><div class="photoGrid">${req.map(x=>photoSlot(x,photos,groups,finalized)).join('')}</div></section>
  ...
  async function uploadPhoto(type,file,addAnother){
    ...
    }catch(err){slot.classList.remove('uploading');slot.classList.add('failed');status.textContent='✕ NOT uploaded';queue.textContent=err.message+' Choose the photo again to retry.';bar.style.width='0%';toast('Photo NOT uploaded','error')}
