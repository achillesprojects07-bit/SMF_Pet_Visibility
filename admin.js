(() => {
  ...
  const state={code:sessionStorage.getItem('smf_admin_code')||'',user:null,dashboard:null,issues:null,poe:null,users:null,system:null,health:null,photoHealth:null,tab:'overview'};
  ...
  async function healthHandshake(){requireApi();const r=await fetch(API+'/api/health',{cache:'no-store',credentials:'omit'});const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.error||'Server health check failed.');return d}

  async function login(code){
    await healthHandshake();
    const r=await api('loginV4',[code]);
    if(!r?.user||r.user.role!=='ADMIN')throw new Error('This page requires an Admin access code.');
    state.code=code;state.user=r.user;sessionStorage.setItem('smf_admin_code',code);$('adminLogout').classList.remove('hidden');$('adminLogin').classList.add('hidden');$('adminApp').classList.remove('hidden');$('adminHello').textContent='Hello, '+(r.user.name||'Admin');$('adminMeta').textContent='ADMIN • '+r.mode+' • Backend '+r.version;await refreshAll();
  }

  $('adminLoginForm').onsubmit=async e=>{e.preventDefault();$('adminLoginError').textContent='';$('adminLoginBtn').disabled=true;try{await login($('adminCode').value.trim())}catch(err){$('adminLoginError').textContent=err.message}finally{$('adminLoginBtn').disabled=false}};
  ...
  function selectTab(tab){state.tab=tab;document.querySelectorAll('.adminTab').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));document.querySelectorAll('.adminPanel').forEach(p=>p.classList.add('hidden'));$('tab_'+tab).classList.remove('hidden')}

  async function refreshAll(){
    $('adminRefresh').disabled=true;
    try{
      const [dashboard,issues,poe,users,system,health,photoHealth]=await Promise.all([
        api('getAdminDashboardV4',[state.code]),api('getAdminIssuesV4',[state.code]),api('getPoeIndexV4',[state.code]),api('getUsersV4',[state.code]),api('getSystemV4',[state.code]),api('healthV4',[state.code]),api('photoUploadHealthV4',[state.code])
      ]);
      Object.assign(state,{dashboard,issues,poe,users,system,health,photoHealth});
      renderOverview();renderStores();renderIssues();renderUsers();renderSystem();
      toast('Admin data refreshed ✓','success');
    }catch(err){toast(err.message,'error')}finally{$('adminRefresh').disabled=false}
  }
  ...
  function renderOverview(){
    ...
    $('tab_overview').innerHTML=`
      ...
      <div class="adminGrid2">
        ...
        <section class="card"><h2>Attention</h2><div class="systemLine"><span>Current issues</span><b>${Number(state.issues?.issues?.length||0)}</b></div><div class="systemLine"><span>Guide review</span><b>${Number(d.review||0)}</b></div><div class="systemLine"><span>Last store sync</span><b>${esc(d.lastSync||'—')}</b></div></section>
      </div>
  ...
  function renderStores(){
    ...
    const teams=[...new Set(rows.map(x=>x.team).filter(Boolean))].sort(),days=[...new Set(rows.map(x=>x.day).filter(Boolean))].sort(),areas=[...new Set(rows.map(x=>x.area).filter(Boolean))].sort();
    $('tab_stores').innerHTML=`<section class="card"><div class="sectionTitle"><h2>Stores & POE</h2><span class="small">Click a store to inspect POE and inventory.</span></div><div class="filterBar"><input id="storeSearch" placeholder="Search store or area"><select id="storeTeam"><option value="">All teams</option>${teams.map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="storeDay"><option value="">All days</option>${days.map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="storeStatus"><option value="">All statuses</option><option>COMPLETED</option><option>INCOMPLETE</option><option>REFUSED</option><option value="CLOSED">STORE CLOSED</option><option value="NOT STARTED">OPEN</option></select></div><div id="storeTableWrap"></div></section><div id="adminStoreDetail"></div>`;
    const paint=()=>{const q=$('storeSearch').value.toLowerCase(),team=$('storeTeam').value,day=$('storeDay').value,status=$('storeStatus').value;const filtered=rows.filter(x=>(!q||(x.name+' '+x.area).toLowerCase().includes(q))&&(!team||x.team===team)&&(!day||x.day===day)&&(!status||x.status===status));$('storeTableWrap').innerHTML=`<div class="adminTableCard"><table class="adminTable"><thead><tr><th>Store</th><th>Team</th><th>Day</th><th>Area</th><th>Status</th><th>Completed</th></tr></thead><tbody>${filtered.map(x=>`<tr class="adminStoreRow" data-key="${esc(x.key)}" style="cursor:pointer"><td><b>${esc(x.name)}</b><div class="small">Stop ${esc(x.stop)} • ${esc(x.category)}</div></td><td>${esc(x.team)}</td><td>${esc(x.day)}</td><td>${esc(x.area)}</td><td><span class="statusDot ${cls(x.status)}"></span><b>${esc(label(x.status))}</b></td><td>${esc(x.completedAt||'—')}</td></tr>`).join('')}</tbody></table></div>`;document.querySelectorAll('.adminStoreRow').forEach(r=>r.onclick=()=>openAdminStore(r.dataset.key))};
    ['storeSearch','storeTeam','storeDay','storeStatus'].forEach(id=>$(id).oninput=paint);paint();
  }

  async function openAdminStore(key){
    $('adminStoreDetail').innerHTML='<section class="card loading">Loading store…</section>';
    try{const r=await api('getAdminStoreV4',[state.code,key]),s=r.store,p=r.poe||{},groups=r.photoGroups||{};const pics=Object.keys(groups).flatMap(k=>(groups[k]||[]).map(x=>({base:k,...x})));$('adminStoreDetail').innerHTML=`<section class="card"><div class="sectionTitle"><div><h2>${esc(s.name)}</h2><div class="small">${esc(s.storeId||'')} • ${esc(s.team)} • ${esc(s.area)} • ${esc(s.category)}</div></div><span class="badge ${cls(p.outcome)}">${esc(label(p.outcome))}</span></div><div class="storeDetail"><div><h3>Visit</h3><div class="systemLine"><span>Finalized by</span><b>${esc(p.completedBy||'—')}</b></div><div class="systemLine"><span>Finalized at</span><b>${esc(p.completedAt||'—')}</b></div><div class="systemLine"><span>Updated</span><b>${esc(p.updatedAt||'—')}</b></div><h3>Notes</h3><p>${esc(p.notes||'—')}</p></div><div><h3>Inventory</h3>${inventoryReadout(s.materials||{},p)}</div></div>${pics.length?`<h3>POE Photos (${pics.length})</h3><div class="photoThumbs">${pics.map(x=>`<a class="photoThumb" href="${esc(x.url||'#')}" target="_blank" rel="noopener"><img src="${esc(x.previewUrl||x.url||'')}" alt="${esc(x.base)}"><b>${esc(x.base)}</b><div class="small">${esc(x.uploadedAt||'')}</div></a>`).join('')}</div>`:'<div class="emptyState">No active POE photos.</div>'}${p.finalized?`<div class="adminActions" style="margin-top:14px"><button id="reopenStore" class="warn">Reopen for Correction</button></div>`:''}</section>`;if($('reopenStore'))$('reopenStore').onclick=()=>reopenStore(key,s.name)}catch(err){$('adminStoreDetail').innerHTML=`<section class="card error">${esc(err.message)}</section>`}
  }
  ...
  async function reopenStore(key,name){const reason=prompt('Reason for reopening '+name+' for correction:');if(!reason)return;try{const r=await api('reopenStoreVisitV4',[state.code,key,reason]);toast('Store reopened. Day submission cleared if required.','success');await refreshAll();selectTab('stores');await openAdminStore(key)}catch(err){toast(err.message,'error')}}

  function renderIssues(){const rows=state.issues?.issues||[];$('tab_issues').innerHTML=`<section class="card"><div class="sectionTitle"><h2>Issues</h2><b>${rows.length}</b></div><p class="small">Operational exceptions detected by the backend. No automatic changes are made here.</p></section>${rows.length?rows.map(x=>`<section class="card issueCard ${String(x.priority||'').toLowerCase()}"><div class="sectionTitle"><h2>${esc(x.name)}</h2><span class="tag">${esc(x.priority)} • ${esc(x.type)}</span></div><p>${esc(x.detail)}</p><div class="issueMeta"><span class="tag">${esc(x.team)}</span><span class="tag">${esc(x.area)}</span></div><div class="adminActions" style="margin-top:10px"><button class="secondary issueOpen" data-key="${esc(x.key)}">Inspect Store</button></div></section>`).join(''):'<section class="card emptyState">No current issues.</section>'}`;document.querySelectorAll('.issueOpen').forEach(b=>b.onclick=()=>{selectTab('stores');openAdminStore(b.dataset.key)})}

  function renderUsers(){const rows=state.users||[];$('tab_users').innerHTML=`<section class="card"><div class="sectionTitle"><h2>Users</h2><button id="newFieldUser">+ Field User</button></div><p class="small">Admin can activate/deactivate field users, move them between teams, or reset their code. In LIVE, users are disabled rather than deleted.</p><div class="adminTableCard"><table class="adminTable"><thead><tr><th>Name</th><th>Role</th><th>Team</th><th>Access Code</th><th>Active</th><th>Last Used</th><th>Actions</th></tr></thead><tbody>${rows.map(u=>`<tr><td><b>${esc(u.name)}</b></td><td>${esc(u.role)}</td><td>${esc(u.team||'—')}</td><td class="userCode">${esc(u.code)}</td><td>${u.active?'✓ Active':'Disabled'}</td><td>${esc(u.last||'—')}</td><td>${u.role==='ADMIN'?'—':`<div class="adminActions"><button class="secondary userToggle" data-row="${u.row}" data-active="${u.active?'1':'0'}">${u.active?'Disable':'Enable'}</button>${u.role==='FIELD'?`<button class="secondary userTeam" data-row="${u.row}" data-team="${esc(u.team)}">Move Team</button><button class="secondary userReset" data-row="${u.row}">Reset Code</button>`:''}</div>`}</td></tr>`).join('')}</tbody></table></div></section>`;
    $('newFieldUser').onclick=createFieldUser;document.querySelectorAll('.userToggle').forEach(b=>b.onclick=()=>toggleUser(b));document.querySelectorAll('.userTeam').forEach(b=>b.onclick=()=>moveTeam(b));document.querySelectorAll('.userReset').forEach(b=>b.onclick=()=>resetCode(b));
  }
  async function createFieldUser(){const name=prompt('Field user name:');if(!name)return;const team=prompt('Assigned team: enter Team 1 or Team 2','Team 1');if(!team)return;try{const r=await api('createUserV4',[state.code,name,team]);alert('New access code: '+r.code+'\n\nGive this code only to the intended field user.');await refreshAll();selectTab('users')}catch(err){toast(err.message,'error')}}
  async function toggleUser(b){try{await api('setUserActiveV4',[state.code,Number(b.dataset.row),b.dataset.active!=='1']);await refreshAll();selectTab('users')}catch(err){toast(err.message,'error')}}
  async function moveTeam(b){const team=b.dataset.team==='Team 1'?'Team 2':'Team 1';if(!confirm('Move this field user to '+team+'? Their access code will be regenerated.'))return;try{const r=await api('setUserTeamV4',[state.code,Number(b.dataset.row),team]);alert('New access code: '+r.code);await refreshAll();selectTab('users')}catch(err){toast(err.message,'error')}}
  async function resetCode(b){if(!confirm('Reset this user access code? The old code will stop working.'))return;try{const r=await api('resetUserCodeV4',[state.code,Number(b.dataset.row)]);alert('New access code: '+r.code);await refreshAll();selectTab('users')}catch(err){toast(err.message,'error')}}

  function renderSystem(){const s=state.system||{},h=state.health||{},p=state.photoHealth||{};$('tab_system').innerHTML=`<div class="adminGrid2"><section class="card"><h2>Backend</h2><div class="systemLine"><span>Mode</span><b>${esc(s.mode||'—')}</b></div><div class="systemLine"><span>Backend version</span><b>${esc(s.version||'—')}</b></div><div class="systemLine"><span>Active stores</span><b>${Number(s.stores||0)}</b></div><div class="systemLine"><span>Last store sync</span><b>${esc(s.lastSync||'—')}</b></div><div class="systemLine"><span>Issues</span><b>${Number(s.issues||0)}</b></div></section><section class="card"><h2>Integrity</h2><div class="systemLine"><span>Store IDs</span><b>${Number(h.storeIds||0)}/${Number(h.stores||0)}</b></div><div class="systemLine"><span>Missing Store IDs</span><b class="${Number(h.missingStoreIds||0)?'badText':'okText'}">${Number(h.missingStoreIds||0)}</b></div><div class="systemLine"><span>Duplicate Store IDs</span><b class="${(h.duplicateStoreIds||[]).length?'badText':'okText'}">${(h.duplicateStoreIds||[]).length}</b></div><div class="systemLine"><span>Safe Sync identity ready</span><b class="${h.safeSyncReady?'okText':'badText'}">${h.safeSyncReady?'YES':'NO'}</b></div></section></div><section class="card"><h2>POE Upload Health</h2><div class="systemLine"><span>Drive root</span><b class="${p.rootFolder?'okText':'badText'}">${p.rootFolder?'CONNECTED':'ERROR'}</b></div><div class="systemLine"><span>V4_PHOTOS sheet</span><b class="${p.photosSheet?'okText':'badText'}">${p.photosSheet?'CONNECTED':'ERROR'}</b></div>${(p.details||[]).map(x=>`<p class="error">${esc(x)}</p>`).join('')}<p class="small">High-risk Store Sync, mode switching, and Demo reset remain intentionally unavailable from this v5 Admin interface during active deployment.</p></section>`}
