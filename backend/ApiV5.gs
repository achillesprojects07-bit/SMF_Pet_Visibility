...
function getBridgeHealthV5(){return {ok:true,bridge:'SMF_API_V5',version:'5.0.1',build:'5.0.4-fast-photo',mode:mode_(),timestamp:now_(),stores:storeRows_().length,photosSheet:!!ss_().getSheetByName(V4.PHOTOS)};}
...
function externalPhotoExistingByTokenV5_(storeKey,note){return rows_(V4.PHOTOS).find(x=>String(x.Environment||'').toUpperCase()===mode_()&&String(x['Store Key']||'')===String(storeKey||'')&&String(x.Notes||'')===String(note||'')&&truth_(x.Active))||null;}
...
function externalPhotoReserveShellV5_(folder,fileName,mime){const lock=LockService.getScriptLock();if(!lock.tryLock(3000))throw new Error('The upload service is busy reserving this photo. Please retry.');try{const matches=folder.getFilesByName(fileName);if(matches.hasNext())return matches.next();return folder.createFile(Utilities.newBlob('',mime||'application/octet-stream',fileName));}finally{try{lock.releaseLock()}catch(_){}}}
