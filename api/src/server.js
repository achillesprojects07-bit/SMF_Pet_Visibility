...
import {authCode,fieldHome,storeDetail,saveDraft,submitStore,submitDay,uploadPhoto,mode} from './domain.js';
...
function auth(req,res,next){
  try{
    ...
    req.user=jwt.verify(raw,process.env.SESSION_SECRET,{issuer:'smf-v5'});
    if(req.user.role!=='FIELD')return res.status(403).json({error:'Not authorized.'});
    next();
...
app.post('/v1/login',async(req,res,next)=>{try{const user=await authCode(req.body?.code);res.json({ok:true,token:sign(user),user,mode:mode()})}catch(e){next(e)}});
